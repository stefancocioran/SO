...
// Insert the given thread in a list containing all threads that have
...
// Removes the first thread from the queue (the biggest priority one)
void pop_queue(void)
{
	unsigned int i;
	// Shift queue elements to the left
	for (i = 0; i < scheduler->queue_size - 1; i++)
...
void push_queue(so_thread_t *thread)
{
	...
	for (i = 0; i < scheduler->queue_size; i++) {
	...
	for (i = scheduler->queue_size - 1; i > index; i--)
		scheduler->ready_queue[i] = scheduler->ready_queue[i - 1];
...
// Runs the next top priority thread
void set_next_running_thread(void)
{
	...
	bRet = ReleaseSemaphore(top_priority_thread->semaphore, 1, NULL);
	DIE(bRet == FALSE, "ReleaseSemaphore failed");
}

// Interrupts the thread that is currently running
void halt_running_thread(so_thread_t *running_thread)
{
	int dwReturn;

	dwReturn = WaitForSingleObject(running_thread->semaphore, INFINITE);
	DIE(dwReturn == WAIT_FAILED, "WaitForSingleObject");
}

// Run the next top priority thread if the time quantum of the currently
// running one has expired or a bigger priority thread has been inserted
// in the priority queue (by executing so_fork or so_signal)
void update_scheduler(void)
{
	so_thread_t *running_thread, *top_priority_thread;
	
	running_thread = scheduler->running_thread;
...
void start_thread(void *arg)
{
	...
	dwReturn = WaitForSingleObject(thread->semaphore, INFINITE);
	DIE(dwReturn == WAIT_FAILED, "WaitForSingleObject");
...
// arg - The argument to pass to start_thread (a thread structure).
...
int so_signal(unsigned int io)
{
	...
	for (i = 0; i < scheduler->list_size; i++) {
...
void so_end(void)
{
	...
	for (i = 0; i < scheduler->list_size; i++) {
		dwReturn = WaitForSingleObject(scheduler->threads[i]->hThread,
			INFINITE);
		DIE(dwReturn == WAIT_FAILED, "WaitForSingleObject");
	}

	for (i = 0; i < scheduler->list_size; i++) {
		CloseHandle(scheduler->threads[i]->semaphore);
